...
class CriticLinearNet(nn.Module):
    def __init__(
            self,
            action_dim: int,
            global_cond_dim: int,
        ...
        self.linear1 = nn.Linear(global_cond_dim + action_dim, 128)
        self.mish1 = nn.Mish()
    ...
    def forward(
        ...
        """
        # linear
        global_cond = torch.cat([global_cond, action], dim=-1)
...
class CriticValueFunctionLinearNet(nn.Module):
    def __init__(
            self,
            global_cond_dim: int,
            dropout_prob: float = 0.2,
...
if __name__ == '__main__':
    ...
    action_dim = 10
    global_cond_dim = 282
    sample = torch.randn(batch_size, action_dim)
    global_cond = torch.randn(batch_size, global_cond_dim)
    network = CriticLinearNet(
