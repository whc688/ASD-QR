...
from scalingup.algo.critics_algo_multiGPU import CriticTrainingAlgo
...
class Critics:
    def __init__(self, critic1: nn.ModuleDict, critic2: nn.ModuleDict):
...
class ActorTrainingAlgo(LightningModule):
    def __init__(
        ...
        self.eta = eta
        self.critics = self.load_critic(critic_checkpoint_path)
    ...
    def load_critic(self, path: str):
        ...
        algo_kwargs['actor_checkpoint_path'] = '/home/kemove/scalingup_original/checkpoints/single_drawer_diffusion_policy_run-20240110_092558-e21qyo0f/checkpoints/epoch_9.ckpt'
        ...
        critic1.eval()
        critic2.eval()
        critics = Critics(critic1, critic2)
        return critics
    ...
    def get_action(
        ...
        if self.policy_task_metric_guidance > 0.0:
    ...
    def get_stats(self, tensor_data: TensorDataClass) -> Dict[str, Any]:
        ...
        new_action = new_action[:, action_start:action_end, :]
    ...
    def on_train_epoch_end(self) -> None:
        ...
        val_loss = np.array(validation_stats['sim/all/mse_loss/mean']).mean()
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: bool = True):
        # state_dict 中的 key 的格式為 ema_nets.state_sequence_encoder 而 self.state_dict() 中的 key 的格式為
        # actor.state_sequence_encoder. 因此需要將 state_dict 中的 key 的格式轉換為 self.state_dict() 中的 key 的格式

        # 加载非 EMA 模型权重
        new_state_dict = {}
        for k, v in state_dict.items():
            if k.startswith('ema_nets.'):
                k = k.replace('ema_nets.', 'actor.')
            new_state_dict[k] = v

        # 加载 EMA 模型权重
        # new_state_dict = state_dict['ema_model']
        # new_state_dict = {"actor." + key: value for key, value in new_state_dict.items()}

        retval = super().load_state_dict(new_state_dict, strict=False)
        print('missing keys:')
        print(retval.missing_keys)
        return retval
    ...
    @property
    def critic1(self):
        return self.critics.critic1

    @property
    def critic2(self):
        return self.critics.critic2
