...
from scalingup.algo.state_encoder.film_conditioning_layer import FilmConditioning
...
def maybe_restore_with_film(*args, weights='imagenet', include_top=True, include_film=False, **kwargs):
    ...
    # 先加载上 top 方便后面加载权重
    n1 = EfficientNet(*args, include_top=True, include_film=False, **kwargs)

    # Load weights.
    if weights == 'imagenet':
        weights_path = os.path.join(os.path.dirname(__file__), 'efficientnet_checkpoints/pytorch_model_b0.bin')
        ...
        if include_top:
            ...
            n1.include_top = False
    else:
        raise ValueError("weights must be either None or 'imagenet'.")
    ...
    n2 = EfficientNet(*args, include_top=True, include_film=True, **kwargs)
    if weights == None:
        return n2
...
def load_official_pytorch_param(model: nn.Module, weights_path):
    ...
    for key_official, key_film_eff in zip(keys_official_list, keys_film_eff_list):
        ...
        if key_official.split('.')[-1] != key_film_eff.split('.')[-1]:
            raise ValueError("key_official.split('.')[-1] != key_film_eff.split('.')[-1]")
