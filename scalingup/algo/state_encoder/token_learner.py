...
class SeqTokenLearnerModule(nn.Module):
    def __init__(self,
                 inputs_channels: int,
                 num_tokens: int,
                 bottleneck_dim: int = 64,
        ...
        self.linear1 = nn.Linear(in_features=inputs_channels,
        ...
        self.linear2 = nn.Linear(in_features=bottleneck_dim,
    ...
    # inputs: [bs, c, l]
    # l is time-series length such as frame
    def forward(self, inputs: torch.Tensor):
        # layer norm
        x = self.layerNorm(inputs.permute(0, 2, 1))
        # x = self.layerNorm(inputs.permute(0, 2, 1)) 后 x: [bs, l, c]

        # create weights map
        x = self.mish1(self.linear1(x))  # x: [bs, l, bottleneck_dim]
        x = self.dropout1(x)
        x = self.linear2(x)  # x: [bs, l, num_tokens]
        x = self.dropout2(x)

        x = x.permute(0, 2, 1)  # x: [bs, num_tokens, l]
        weights_maps = F.softmax(x, dim=-1)  # weights_maps: [bs, num_tokens, l]

        # create tokens
        inputs = inputs.permute(0, 2, 1)  # inputs: [bs, l, c]

        tokens = torch.bmm(weights_maps, inputs)  # tokens: [bs, num_tokens, c]
