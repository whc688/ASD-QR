...
class SingleEnvSimEvaluation(SimEvaluation):
    def __init__(self, env: Env, task_sampler: TaskSampler, gui: bool = False, **kwargs):
        self.env = env
        ...
        if self.remote:
            ...
            if self.gui:
                ...
                viewer_handle = viewer.launch_passive(
                    self.env.mj_physics.model.ptr, self.env.mj_physics.data.ptr
                )
                ...
                self.env.step_fn_callbacks["viewer_sync_callback"] = (
            ...
            for episode_id in episode_ids:
                try:
                    # 打印当前进行了多少个 episode
                    print('total episode: ', len(episode_ids), ' ',
                          'finished episodes: ', len(trajectory_summary_dataframes))
