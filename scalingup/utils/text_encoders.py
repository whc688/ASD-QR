...
class ClipLanguageConditioned(torch.nn.Module):
    ...
    def get_clip_model_and_processor(
        ...
        if cls.clip_model is None:
            ...
            cls.clip_model = CLIPModel.from_pretrained('./clip-vit-base-patch32/').to(device)  # type: ignore
            for param in cls.clip_model.parameters():
                param.requires_grad = False
            cls.clip_model.eval()
            cls.clip_processor = CLIPProcessor.from_pretrained('./clip-vit-base-patch32/')
...
class DictCacheClipLanguageConditioned(torch.nn.Module):
    clip_model: Optional[CLIPModel] = None
    clip_processor: Optional[CLIPProcessor] = None
    ...
    def __init__(
        ...
        super().__init__()
        self.lang_embs_cache = {}  # 使用普通的 Python 字典
        self.use_one_hot = use_one_hot
        self.use_last_hidden_state = use_last_hidden_state
        self.max_descs = max_descs
        self.text_dim = text_dim
        ...
        if text not in self.lang_embs_cache:
            ...
            if self.use_one_hot:
                ...
                with torch.no_grad():
                    ...
                    # 注册 buffer
                    buffer_name = f'lang_emb_{text}'
                    self.register_buffer(buffer_name, emb)

                    # 在普通字典中存储 buffer 名称
                    self.lang_embs_cache[text] = buffer_name
        # 通过 buffer 名称访问实际的嵌入
        buffer_name = self.lang_embs_cache[text]
